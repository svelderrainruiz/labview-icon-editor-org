...
Describe 'VI Analyzer contract' {
    ...
    It 'enforces non-zero analyzed tests and file-level failure extraction in Run-ViAnalyzer' {
        ...
        $content | Should -Match 'Test-ViAnalyzerMissingInstallSignature'
        $content | Should -Match '-350053'
        $content | Should -Match 'missing\\s\+or\\s\+bad\\s\+files'
        $content | Should -Match 'installation_guard'
        $content | Should -Match 'VI Analyzer installation guard failed'
